...
const CalendarAddEventModal = ({
  ...
  const getCustomersResults = (textCustomer) => {
    axios
      .get(API_URL + ApiRoutes.search_customer + textCustomer)
      .then((res) => {
  ...
  const getEstatesResults = (textEstate) => {
    axios
      .get(API_URL + ApiRoutes.search_estates + "/" + textEstate)
      .then((res) => {
  ...
  const getApptmtType = () => {
    axios
      .get(API_URL + ApiRoutes.apptmtType)
      .then((res) => {
  ...
  const onSelectedEstate = (item) => {
    ...
    setTextEstate(
      item.title + "- " + item.city + " " + item.zipcode + " " + item.reference
    );
  };
  ...
  const insertAppointment = (datas) => {
    axios
      .post(API_URL + ApiRoutes.create_apptmt, datas)
      .then((res) => {
          ...
          <Formik
            ...
            validationSchema={Yup.object({
              ...
              id_customer: Yup.string().required(
                "Veuillez sélectionner un client/contact ou en créer un"
              ),
              id_appointment_type: Yup.string().required(
            ...
            onSubmit={async (values) => {
              const scheduled_at =
                (values.date ? values.date : dateValue) +
                " " +
                (values.time ? values.time : timeValue) +
                ":00";
              let data = {
                ...values,
                ...
                <form>
                  ...
                  <div className="row">
                    ...
                    <div className="col m-3">
                      ...
                      >
                        {errors.id_appointment_type ?? null}
                      </div>
                    </div>
                  </div>
                  <div className="row">
